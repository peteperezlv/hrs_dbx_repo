...
SELECT sr.hrs_survey_respondent_id,
...
FROM unpivoted_age_data ua
    INNER JOIN dev_catalog.slv_cdm_hrs.hrs_survey_variable sv ON ua.survey_variable_name = sv.survey_variable_name
    INNER JOIN dev_catalog.slv_cdm_hrs.hrs_survey_respondent sr ON CAST(CAST(ua.hhidpn AS BIGINT) AS STRING) = sr.hhidpn
    INNER JOIN dev_catalog.slv_cdm_hrs.hrs_cohort c ON CAST(ua.HACOHORT AS BIGINT) = c.hacohort_number
