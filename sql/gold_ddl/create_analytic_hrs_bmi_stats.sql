...
CREATE TABLE dev_catalog.gld_star_hrs.analytic_hrs_bmi_stats (
    ...
    wave_id BIGINT NOT NULL COMMENT 'Foreign key to the HRS wave dimension',
    cohort_id BIGINT NOT NULL COMMENT 'Foreign key to the HRS cohort dimension',
    ...
    bmi_min INT COMMENT 'Minimum BMI',
    bmi_max INT COMMENT 'Maximum BMI',
    ...
    CONSTRAINT fk_analytic_hrs_bmi_stats_dim_wave
        FOREIGN KEY (wave_id)
        REFERENCES dev_catalog.slv_cdm_hrs.dim_wave (wave_id)
        NOT ENFORCED,
    CONSTRAINT fk_analytic_hrs_bmi_stats_dim_cohort
        FOREIGN KEY (cohort_id)
        REFERENCES dev_catalog.slv_cdm_hrs.dim_cohort (cohort_id)
        NOT ENFORCED
)
USING DELTA
COMMENT 'BMI descriptive statistics grouped by HRS cohort, wave, race, gender, and age';
