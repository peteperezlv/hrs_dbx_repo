-- ============================================================================
-- HRS Survey Response Age Data ETL Validation Script.  Test a change and sync to databricks.
-- ============================================================================
...
WITH source_unpivoted AS (
    SELECT CAST(CAST(hhidpn AS BIGINT) AS STRING) AS hhidpn,
...
SELECT 'Test 1: Source Record Count' AS test_name,
...
target_count AS (
    ...
    FROM dev_catalog.slv_cdm_hrs.hrs_survey_response sr
        INNER JOIN dev_catalog.slv_cdm_hrs.hrs_survey_variable sv ON sr.hrs_survey_variable_id = sv.hrs_survey_variable_id
...
missing_variables AS (
    ...
    FROM expected_variables ev
        LEFT JOIN dev_catalog.slv_cdm_hrs.hrs_survey_variable sv ON ev.variable_name = sv.survey_variable_name
    WHERE sv.survey_variable_name IS NULL
)
SELECT 'Test 3: Missing Survey Variables' AS test_name,
...
missing_respondents AS (
    ...
    FROM source_respondents sr
        LEFT JOIN dev_catalog.slv_cdm_hrs.hrs_survey_respondent resp ON sr.hhidpn = resp.hhidpn
...
WITH source_cohorts AS (
    SELECT DISTINCT CAST(HACOHORT AS BIGINT) AS hacohort_number
    FROM dev_catalog.brz_raw_hrs.randhrs1992_2022v1
),
missing_cohorts AS (
...
WITH duplicate_check AS (
    ...
    GROUP BY hrs_survey_respondent_id,
        hrs_survey_variable_id
    HAVING COUNT(*) > 1
)
SELECT 'Test 6: Duplicate Responses' AS test_name,
...
SELECT 'Test 7: NULL Value Validation' AS test_name,
    COUNT(*) AS null_count,
    CASE
        WHEN COUNT(*) = 0 THEN 'PASS'
        ELSE 'FAIL'
...
WITH orphan_check AS (
    ...
    SELECT 'hrs_survey_variable_id' AS fk_column,
        COUNT(*) AS orphan_count
    FROM dev_catalog.slv_cdm_hrs.hrs_survey_response sr
        LEFT JOIN dev_catalog.slv_cdm_hrs.hrs_survey_variable sv ON sr.hrs_survey_variable_id = sv.hrs_survey_variable_id
    WHERE sv.hrs_survey_variable_id IS NULL
    UNION ALL
    -- Check hrs_survey_respondent_id
    SELECT 'hrs_survey_respondent_id' AS fk_column,
        COUNT(*) AS orphan_count
    FROM dev_catalog.slv_cdm_hrs.hrs_survey_response sr
        LEFT JOIN dev_catalog.slv_cdm_hrs.hrs_survey_respondent resp ON sr.hrs_survey_respondent_id = resp.hrs_survey_respondent_id
    WHERE resp.hrs_survey_respondent_id IS NULL
    UNION ALL
    -- Check hrs_wave_id
    SELECT 'hrs_wave_id' AS fk_column,
        COUNT(*) AS orphan_count
    FROM dev_catalog.slv_cdm_hrs.hrs_survey_response sr
        LEFT JOIN dev_catalog.slv_cdm_hrs.hrs_wave w ON sr.hrs_wave_id = w.hrs_wave_id
    WHERE w.hrs_wave_id IS NULL
    UNION ALL
    -- Check hrs_cohort_id
    SELECT 'hrs_cohort_id' AS fk_column,
        COUNT(*) AS orphan_count
    FROM dev_catalog.slv_cdm_hrs.hrs_survey_response sr
        LEFT JOIN dev_catalog.slv_cdm_hrs.hrs_cohort c ON sr.hrs_cohort_id = c.hrs_cohort_id
    WHERE c.hrs_cohort_id IS NULL
)
SELECT 'Test 9: Referential Integrity' AS test_name,
...
FROM dev_catalog.slv_cdm_hrs.hrs_survey_response sr
    INNER JOIN dev_catalog.slv_cdm_hrs.hrs_survey_variable sv ON sr.hrs_survey_variable_id = sv.hrs_survey_variable_id
    INNER JOIN dev_catalog.slv_cdm_hrs.hrs_survey_respondent resp ON sr.hrs_survey_respondent_id = resp.hrs_survey_respondent_id
    INNER JOIN dev_catalog.slv_cdm_hrs.hrs_wave w ON sr.hrs_wave_id = w.hrs_wave_id
    INNER JOIN dev_catalog.slv_cdm_hrs.hrs_cohort c ON sr.hrs_cohort_id = c.hrs_cohort_id
WHERE sv.survey_variable_name LIKE 'R%AGEY_E'
ORDER BY resp.hhidpn,
    sv.survey_variable_name
LIMIT 25;